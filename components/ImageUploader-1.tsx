    ...
    <div className="w-full relative">
      {isLoading && (
        <div className="absolute inset-0 bg-white/80 backdrop-blur-sm flex flex-col justify-center items-center z-10 rounded-lg">
            ...
            <p className="mt-3 text-slate-700 font-semibold">{loadingMessage}</p>
        </div>
      )}
      {preview ? (
        <div className="relative w-full h-48 rounded-lg overflow-hidden border border-slate-300">
          <img src={preview} alt="Preview" className="w-full h-full object-cover" />
           <button
            ...
            className="absolute top-2 right-2 bg-black/50 text-white rounded-full p-1.5 hover:bg-black/75 transition-colors focus:outline-none focus:ring-2 focus:ring-white"
